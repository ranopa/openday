...
<script type="text/javascript">
	...
	document
			.addEventListener(
					"DOMContentLoaded",
					function() {
						const sliderContainerHot = document
								.querySelector(".slider-container-hot");
						const sliderListHot = document
								.querySelector(".slider-list-hot");
						const prevBtnHot = document
								.querySelector(".slider-prev-btn-hot");
						const nextBtnHot = document
								.querySelector(".slider-next-btn-hot");

						const itemWidthHot = sliderListHot
								.querySelector(".oclassHot").offsetWidth;
						const containerWidthHot = sliderContainerHot.offsetWidth;
						const slideCountHot = sliderListHot
								.querySelectorAll(".oclassHot").length;
						const maxPositionHot = itemWidthHot * slideCountHot
								- containerWidthHot; // 보정값 미적용
						...
						nextBtnHot.addEventListener("click", function() {
							if (currentPositionHot < maxPositionHot) {
								...
								sliderListHot.style.transform = "translateX(-"
										+ (currentPositionHot + adjustmentHot)
										+ "px)"; // 보정값 추가
							}
						});
	...
	document
			.addEventListener(
					"DOMContentLoaded",
					function() {
						const sliderContainerNew = document
								.querySelector(".slider-container-new");
						const sliderListNew = document
								.querySelector(".slider-list-new");
						const prevBtnNew = document
								.querySelector(".slider-prev-btn-new");
						const nextBtnNew = document
								.querySelector(".slider-next-btn-new");

						const itemWidthNew = sliderListNew
								.querySelector(".oclassNew").offsetWidth;
						const containerWidthNew = sliderContainerNew.offsetWidth;
						const slideCountNew = sliderListNew
								.querySelectorAll(".oclassNew").length;
						const maxPositionNew = itemWidthNew * slideCountNew
								- containerWidthNew; // 보정값 미적용
						...
						nextBtnNew.addEventListener("click", function() {
							if (currentPositionNew < maxPositionNew) {
								...
								sliderListNew.style.transform = "translateX(-"
										+ (currentPositionNew + adjustmentNew)
										+ "px)"; // 보정값 추가
							}
						});
	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		nextBtnD.addEventListener("click", function() {
			if (currentPositionD < maxPositionD) {
				...
				sliderListD.style.transform = "translateX(-"
						+ (currentPositionD + adjustmentD) + "px)"; // 보정값 추가
			}
		});
		...
		prevBtnD.addEventListener("click", function() {
			if (currentPositionD > 0) {
				currentPositionD -= itemWidthD;
				sliderListD.style.transform = "translateX(-" + currentPositionD
						+ "px)";
			}
		});
	});
	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		nextBtnR.addEventListener("click", function() {
			if (currentPositionR < maxPositionR) {
				...
				sliderListR.style.transform = "translateX(-"
						+ (currentPositionR + adjustmentR) + "px)"; // 보정값 추가
			}
		});
		...
		prevBtnR.addEventListener("click", function() {
			if (currentPositionR > 0) {
				currentPositionR -= itemWidthR;
				sliderListR.style.transform = "translateX(-" + currentPositionR
						+ "px)";
			}
		});
	});
...
<body>
									...
									<c:forEach items="${plist}" var="list" varStatus="loop">
										<c:if test="${loop.index < 12}">
											<a href="classinfo/${list.clsId}">
												<li class="oclass oclassP"><img
													src="img/${list.filNum}" class="ssum-img">
													<div class="txt-box">
														<div class="tb1">
															<div class="t1">
	...
	<button class="slider-next-btn-hot"
		style="border: none; background-color: transparent;">
		<span class="material-symbols-outlined" style="font-size: 35px;">
			chevron_right </span>
	</button>
 ...
 --%>
	...
	<button class="slider-next-btn-new"
		style="border: none; background-color: transparent;">
		<span class="material-symbols-outlined" style="font-size: 35px;">
			chevron_right </span>
	</button>
	...
	<a href="classOpen"> <img
		src="resources/image/user/cooksmallbanner.jpg" alt="이미지"
		class="smallBanner"></a>
	...
	<button class="slider-next-btn-d"
		style="border: none; background-color: transparent;">
		<span class="material-symbols-outlined" style="font-size: 35px;">
			chevron_right </span>
	</button>
	...
	<button class="slider-next-btn-r"
		style="border: none; background-color: transparent;">
		<span class="material-symbols-outlined" style="font-size: 35px;">
			chevron_right </span>
	</button>
